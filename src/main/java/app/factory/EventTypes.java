package app.factory.h20;

public class EventTypes {
    public static final String BaseEvent = "BASE_EVENT";
    public static final String ArrivalEvent = "ARRIVAL_EVENT";
    public static final String EndReceptionEvent = "END_TRASMISSION_EVENT";
    public static final String MoveEvent = "MOVE_EVENT";
    public static final String StatisticEvent = "STATISTIC_EVENT";
    public static final String TransmissionEvent = "TRASMISSION_EVENT";
    public static final String ReceivingTransmissionEvent = "RECEIVING_TRANSMISSION_EVENT";
    public static final String RetransmitEvent = "RETRANSMIT_EVENT";
    public static final String EndTransmissionEvent = "EndTransmissionEvent";


}
